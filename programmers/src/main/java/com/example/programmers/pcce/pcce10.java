...
public class pcce10 {
    ...
    //예를 들어 다음과 같이 데이터가 주어진다면 data = [[1, 20300104, 100, 80], [2, 20300804, 847, 37], [3, 20300401, 10, 8]]
    ...
    public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        ...
        List<int[]> valExtData = new ArrayList<>();
        for (int[] i : data){
            if (i[dataType.get(ext)] < val_ext){
                valExtData.add(i);
            }
        }
        valExtData.sort(Comparator.comparingInt(arr -> arr[dataType.get(sort_by)]));

        return valExtData.toArray(new int[0][]);
    }
}
//ext = code, date, maximum, remain
//ext의 값이 val_ext 보다 작아야
//이 값들을 sort_by 기준 오름차순